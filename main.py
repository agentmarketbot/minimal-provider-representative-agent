...
def run_market_scan():
    while True:
        try:
            ...
            logger.error(f"Market scan iteration failed with error: {e.__class__.__name__}: {str(e)}", exc_info=True)
            time.sleep(10)
...
def run_solve_instances():
    while True:
        try:
            ...
            logger.error(f"Solve instances iteration failed with error: {e.__class__.__name__}: {str(e)}", exc_info=True)
            time.sleep(10)
...
def main():
    ...
    try:
        ...
        logger.error(f"Fatal error in main loop: {e.__class__.__name__}: {str(e)}", exc_info=True)
        market_scan_process.terminate()
        solve_instances_process.terminate()
        sys.exit(1)
